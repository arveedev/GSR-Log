...
function App() {
    ...
    // Access loading, error, and now 'data' from the global data context.
    // NOTE: The 'exportData' function is no longer destructured here.
    const { loading, error, data } = useAppData();
    ...
    const renderTab = () => {
        // Pass the 'data' prop to each component
        switch (activeTab) {
            case 'logEntry':
                return <LogEntry sortedData={data} />;
            case 'logHistory':
                return <LogHistory sortedData={data} />;
            case 'dailySummary':
                return <DailySummary sortedData={data} />;
            case 'manageData':
                return <ManageData sortedData={data} />;
            // NEW: Add a case to render the PalayDelivery component.
            case 'palayDelivery':
                return <PalayDelivery sortedData={data} />;
            default:
                return <LogEntry sortedData={data} />;
        }
    };
